...
public class ArgsParser {
    ...
    protected void validateArgs() throws InvalidArgsException {
        ...
        for (Character flag : illegalFlags) {
            String exceptionMessage = ERR_INVALID_FLAG + ": " + flag;
    ...
    /**
     * Construct the full path to the file/folder specified by an argument
     *
     * @param arg Local path to a file/folder
     * @return
     */
    protected String absolutePathArg(String arg) {
        if (Paths.get(arg).isAbsolute()) {
            return arg;
        }
        return Path.of(Environment.currentDirectory).resolve(arg).toString();
    }
}
